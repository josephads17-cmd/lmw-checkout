...
const BULK_DISCOUNT_MIN_QUANTITY = 8;
const BULK_DISCOUNT_PERCENT_OFF = 15;
...
async function resolveBulkDiscountCoupon(isMonthly) {
  const id = isMonthly ? 'lmw-bulk15-forever' : 'lmw-bulk15-once';
  try {
  ...
  const coupon = await stripe.coupons.create({
    id,
    percent_off: BULK_DISCOUNT_PERCENT_OFF,
    duration: isMonthly ? 'forever' : 'once',
    name: `Réduction ${BULK_DISCOUNT_PERCENT_OFF}% dès ${BULK_DISCOUNT_MIN_QUANTITY} sachets`,
  });
  return coupon.id;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const bulkDiscountEligible = productQuantity >= BULK_DISCOUNT_MIN_QUANTITY;
    ...
    const metadata = {
      ...
      bulk_discount_applied: String(bulkDiscountEligible),
    ...
    if (bulkDiscountEligible) {
      const couponId = await resolveBulkDiscountCoupon(isMonthly);
