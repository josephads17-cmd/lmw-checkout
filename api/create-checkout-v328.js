// api/create-checkout-v328.js
//
// Checkout dédié à la Beta V3.28 : offres précomposées, livraison offerte
// dès 4 sachets et remise automatique à partir de 8 sachets.
...
const DEFAULT_CANCEL_URL = 'https://lamaisonwinnie.com/beta-v3-28.html?checkout=cancelled#composer';
...
function normalizeItems(items, requiredQuantity = null) {
  if (!Array.isArray(items) || items.length === 0) throw new Error('Ajoutez au moins un produit à la box.');
  const normalized = [];
  let totalQuantity = 0;
...
async function resolveAutomaticDiscount() {
  ...
  try {
    const promotionCodes = await stripe.promotionCodes.list({ code: AUTOMATIC_DISCOUNT_REFERENCE, active: true, limit: 1 });
...
function sharedSessionConfig({ normalizedName, cancelUrl, metadata }) {
  return {
    custom_fields: [rabbitNameField(normalizedName)], metadata,
  ...
  try {
    ...
    if (bundleId) {
      ...
      const composition = bundle.requiredQuantity
        ...
        : bundle.composition;
      const metadata = {
        rabbit_name: normalizedName || 'À renseigner dans Checkout', delivery_mode: 'one_time',
        shipping_free: 'true', bundle_id: String(bundleId), bundle_name: bundle.name,
        composition: JSON.stringify(composition),
      };
      const session = await stripe.checkout.sessions.create({
        ...sharedSessionConfig({ normalizedName, cancelUrl, metadata }),
        mode: 'payment', line_items: [{ price: bundle.price, quantity: 1 }], allow_promotion_codes: true,
      });
    ...
    const automaticDiscount = normalized.totalQuantity >= 8 ? await resolveAutomaticDiscount() : null;
    ...
    const metadata = {
      rabbit_name: normalizedName || 'À renseigner dans Checkout', delivery_mode: deliveryMode,
      shipping_free: String(shippingIsFree), automatic_discount: String(Boolean(automaticDiscount)),
      composition: JSON.stringify(normalized.items),
    };
    const sessionConfig = {
      ...sharedSessionConfig({ normalizedName, cancelUrl, metadata }),
      mode: isMonthly ? 'subscription' : 'payment', line_items: lineItems,
    };
    ...
    if (!isMonthly && !shippingIsFree) {
      sessionConfig.shipping_options = [{
        shipping_rate_data: {
          display_name: 'Livraison standard', type: 'fixed_amount',
          fixed_amount: { amount: 499, currency: 'eur' }, tax_behavior: 'inclusive', tax_code: 'txcd_92010001',
        },
      }];
    }
    ...
    console.error('Erreur création session Checkout Beta V3.28:', err);
